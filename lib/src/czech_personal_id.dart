...
bool _isIdValid(final String id) {
  ...
  // validate year
  final y = int.parse(id.trim().substring(0, 2));
  if (!(y >= 0 && y <= 99)) return false;

  // validate month
  final m = int.parse(id.trim().substring(2, 4));
  final mNormalized = _normalizeMonth(m);
  if (mNormalized < 1 || mNormalized > 12) return false;

  // validate date
  final d = int.parse(id.trim().substring(4, 6));
  return _isValidDate(d, mNormalized, 2000 + y);
}

bool _isValidDate(final int d, final int m, final int yyyy) {
  try {
    final dt = DateTime(yyyy, m, d);
    return dt.month == m;
  } catch (e) {
    return false;
  }
...
bool _isSuffixValid(final String id) {
  ...
  if (suffix.length == 4) {
    var firstNine = int.parse(id.substring(0, 9));
    var last = int.parse(id[9]);
    var result = firstNine % 11;
    if (result == 10) {
